      ...
      >
        <div className="max-w-7xl mx-auto px-8 sm:px-12 lg:px-16">
          <div className="flex items-center justify-between h-16 lg:h-20">
            ...
            <div className="hidden md:flex items-center gap-8">
              {navLinks.map((link) => (
                <a
                  key={link.name}
                  href={link.href}
                  className="text-sm font-medium text-gray-warm hover:text-charcoal transition-colors duration-300 relative group"
                >
                  {link.name}
            ...
            <a
              href="#contact"
              className="hidden md:inline-flex items-center gap-2 px-5 py-2.5 bg-charcoal text-cream text-sm font-medium rounded-full hover:bg-amber transition-colors duration-300"
            >
              Let's Talk
          ...
          >
            {navLinks.map((link, i) => (
              <motion.a
                ...
                className="text-3xl font-serif font-bold text-charcoal hover:text-amber transition-colors"
              >
                {link.name}
              </motion.a>
            ))}
